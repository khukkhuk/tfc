...
class ProductController extends Controller
{
    ...
    public function datatable(Request $request)
    {
        ...
        return Datatables::of($sTable)
            ...
            ->addColumn('action', function ($row) {
                return " 
                <a href='$this->segment/$this->folder/view/$row->id' class='mr-3 btn btn-sm btn-instagram ' title='Edit'><i class='fa fa-search w-4 h-4 mr-1'></i> View </a>                                                 
    ...
    public function destroy(Request $request)
    {
        ...
        if (@$datas) {
            ...
            {
                ModelModel::where('product_id',$data->product_id)->delete();
    ...
    public function store_modal($request, $id = null)
    {
        try {
            ...
            if ($data->save()) {
                DB::commit();
                return view("$this->prefix.alert.success", ['url' => url("$this->segment/$this->folder/edit/$data->product_id")]);
            } else {
                return view("$this->prefix.alert.error", ['url' => url("$this->segment/$this->folder/edit/$request->product_id")]);
            }
        } catch (\Exception $e) {
            $error_log = $e->getMessage();
            $error_line = $e->getLine();
